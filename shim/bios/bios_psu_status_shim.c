...
static override_symbol_inst *SA6400I2CGetPowerInfo_ovs = NULL;
...
static int SA6400I2CGetPowerInfo_shim(void)
{
    return 1;
}
...
int register_bios_psu_status_shim(const struct hw_config *hw)
{
    ...
    if (unlikely(FS2500I2CGetPowerInfo_ovs))
        shim_reg_already();
    if (unlikely(SA6400I2CGetPowerInfo_ovs))
        shim_reg_already();
    ...
    override_symbol_or_exit_int(SA6400I2CGetPowerInfo_ovs, "SA6400I2CGetPowerInfo_ovs", SA6400I2CGetPowerInfo_shim);    
...
int unregister_bios_psu_status_shim(void)
{
    ...
    int outsa6400 = restore_symbol(SA6400I2CGetPowerInfo_ovs);
    if (unlikely(outsa6400 != 0)) {
        pr_loc_err("Failed to restore SA6400I2CGetPowerInfo - error=%d", out);
        return out;
    }
    HWMONGetPSUStatusByI2C_ovs = NULL;
    RS4021xspI2CGetPowerInfo_ovs = NULL;
    FS2500I2CGetPowerInfo_ovs = NULL;
    SA6400I2CGetPowerInfo_ovs = NULL;
...
int reset_bios_psu_status_shim(void)
{
    ...
    put_overridden_symbol(SA6400I2CGetPowerInfo_ovs);
    SA6400I2CGetPowerInfo_ovs = NULL;
